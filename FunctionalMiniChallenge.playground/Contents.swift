...
let freshmanOnly = students.filter { $0.year == StudentYear.freshman}
...
let fullNames = students.map{(studentName: Student)-> String in
    return studentName.firstName + " " + studentName.lastName
}

print(fullNames)

let total = (students.reduce(0){(result, student) -> Int in
